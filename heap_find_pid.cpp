	...
	void dump()
	{
		for (uint32_t i=0; i<m_size; ++i)
		{
			// printf ("task = 0x%4.4x: baton = %p, type = %u, ptr_addr = 0x%llx + 0x%llu\n", task, baton, type, ptr_addr, ptr_size);
                                    printf("match, addr:%p, size:0x%x, offset:0x%x, type:%d\n", m_entries[i].addr, m_entries[i].size, m_entries[i].offset, m_entries[i].type);
		}
	}
...
task_peek (task_t task, vm_address_t remote_address, vm_size_t size, void **local_memory)
{
    ...
    // kern_return_t kr = mach_vm_read(task,        // vm_map_t target_task,
    //                            remote_address,     // mach_vm_address_t address,
    //                            size,     // mach_vm_size_t size
    //                            (vm_offset_t*)local_memory,     //vm_offset_t *data,
    //                            &len);     // mach_msg_type_number_t *dataCnt
    // printf("task:%d, remote:%p, size:%d, local:%p, size:%d\n", task, remote_address, size, local_memory, len);
    // return kr;
...
foreach_zone_in_this_process (range_callback_info_t *info)
{
    ...
    kern_return_t err = malloc_get_all_zones (task, task_peek, &zones, &num_zones);
...
{
    printf ("task = %d: baton = %p, type = %u, ptr_addr = 0x%llx + %llu\n", task, baton, type, ptr_addr, ptr_size);
}
...
ranges_callback (task_t task, void *baton, unsigned type, vm_range_t *ptrs, unsigned count) 
{
    ...
    while(count--) {
        dump_malloc_block_callback(task, info->baton, type, ptrs->address, ptrs->size);
